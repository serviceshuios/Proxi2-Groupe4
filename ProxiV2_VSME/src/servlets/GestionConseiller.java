...
>>>>>>> branch 'master' of https://github.com/serviceshuios/Proxi2-Groupe4.git
							...
							if (request.getParameter("telephone") == null
								...
								if (request.getParameter("adresse") == null
									...
									if (request.getParameter("codepostal") == null
										...
										// else{
										// if
										// (request.getParameter("idclientform")
										// == null
										// ||
										// request.getParameter("idclientform").equals(""))
										// {
										...
										// }
											...
											if (validform == false) {
												request.setAttribute("validerModifierclientdefaut", "pb");
												request.getRequestDispatcher("/ModifierClient.jsp").forward(request,
														response);
											} else {
												// sinon on envoie le tout en
												// base de données
												IConseiller ic = new Services();
												int idcli = Integer
														.parseInt(request.getParameter("idclientform").toString());
												Collection<Client> colcli = ic.listerClient(Integer
														.parseInt(session.getAttribute("idConseiller").toString()));
												for (Client client : colcli) {
													if (client.getIdClient() == idcli) {
														...
														adr.setVille(request.getParameter("ville"));
														ic.modifierClient(client, request.getParameter("nom"),
																request.getParameter("prenom"), adr,
																request.getParameter("email"));
													}
												}
												request.setAttribute("resultatvalidation", "client modifié");
												request.getRequestDispatcher("/interfaceConseiller.jsp")
		 ...
		 * Effectuer un virement
		 */
		// selection du compte créditeur
		if (request.getParameter("effectuervirementphase1") != null) {
			...
			if (request.getParameter("idclientform") != null) {
				...
				for (Client client : colcli1) {
					if (client.getIdClient() == idcli) {
						Collection<Compte> colcom = ic1.listerCompteClient(client);
						request.setAttribute("listecomptesclientcrediteur", colcom);
		...
		if (request.getParameter("effectuervirementphase2") != null) {
			if (request.getParameter("idcompteform") != null) {
				...
				for (Client client : colcli1) {
					if (client.getIdClient() == idcli) {
						Collection<Compte> colcom = ic1.listerCompteClient(client);
						request.setAttribute("listecomptesclientdebiteur", colcom);
						request.getRequestDispatcher("/virementcompteacompte.jsp").forward(request, response);
					}
				}
			}
